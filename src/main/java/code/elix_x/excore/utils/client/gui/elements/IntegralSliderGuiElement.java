...
public class IntegralSliderGuiElement<H extends IGuiElementsHandler<? extends IGuiElement<H>>> extends RectangularGuiElement<H> {
	...
	public IntegralSliderGuiElement(String name, int xPos, int yPos, int width, int height, int borderX, int borderY, int sliderWidth, int min, int max, int defaultValue, boolean displayValue){
		...
		this.min = min;
		this.max = max;
		this.value = (defaultValue - min) / (double) (max - min);
