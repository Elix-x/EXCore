...
public class WebResourcePack extends ExternalDefaultRP {
	...
	public boolean resourceExists(ResourceLocation location){
		return domains.contains(location.getResourceDomain()) && getURL.apply(location.getResourcePath()).map(urlExists::test).orElse(false);
	}

	@Override
	public InputStream getInputStream(ResourceLocation location) throws IOException{
		return getURL.apply(location.getResourcePath()).orElseThrow(() -> new IOException(String.format("Could not load %s from web RP %s", location, getPackName()))).openStream();
	}
