...
public abstract class GuiElement<H extends IGuiElementsHandler<? extends IGuiElement<H>>> implements IGuiElement<H> {
	...
	public void drawColoredRect(Rectangle element, RGBA color, double zLevel){
		...
		WorldRenderer.pos(element.getX(), bottom(element), zLevel).endVertex();
		WorldRenderer.pos(right(element), bottom(element), zLevel).endVertex();
		WorldRenderer.pos(right(element), element.getY(), zLevel).endVertex();
		WorldRenderer.pos(element.getX(), element.getY(), zLevel).endVertex();
		tessellator.draw();
	...
	public void drawTexturedRect(Rectangle element, Rectangle texture, int textureWidth, int textureHeight, double zLevel){
		...
		WorldRenderer.pos(element.getX(), bottom(element), zLevel).tex(texture.getX() * uScale, bottom(texture) * vScale).endVertex();
		WorldRenderer.pos(right(element), bottom(element), zLevel).tex(right(texture) * uScale, bottom(texture) * vScale).endVertex();
		WorldRenderer.pos(right(element), element.getY(), zLevel).tex(right(texture) * uScale, texture.getY() * vScale).endVertex();
		WorldRenderer.pos(element.getX(), element.getY(), zLevel).tex(texture.getX() * uScale, texture.getY() * vScale).endVertex();
		tessellator.draw();
		GlStateManager.disableBlend();
	}
