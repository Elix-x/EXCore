...
public abstract class GuiElement<H extends IGuiElementsHandler<? extends IGuiElement<H>>> implements IGuiElement<H> {
	...
	public void drawColoredRect(Rectangle element, RGBA color, double zLevel){
		...
		tessellator.addVertex(element.getX(), bottom(element), zLevel);
		tessellator.addVertex(right(element), bottom(element), zLevel);
		tessellator.addVertex(right(element), element.getY(), zLevel);
		tessellator.addVertex(element.getX(), element.getY(), zLevel);
		tessellator.draw();
	...
	public void drawTexturedRect(Rectangle element, Rectangle texture, int textureWidth, int textureHeight, double zLevel){
		...
		tessellator.startDrawingQuads();
		tessellator.addVertexWithUV(element.getX(), bottom(element), zLevel, texture.getX() * uScale, bottom(texture) * vScale);
		tessellator.addVertexWithUV(right(element), bottom(element), zLevel, right(texture) * uScale, bottom(texture) * vScale);
		tessellator.addVertexWithUV(right(element), element.getY(), zLevel, right(texture) * uScale, texture.getY() * vScale);
		tessellator.addVertexWithUV(element.getX(), element.getY(), zLevel, texture.getX() * uScale, texture.getY() * vScale);
		tessellator.draw();
		GL11.glDisable(GL11.GL_BLEND);
	}
	...
	public void drawString(FontRenderer font, String text, int x, int y, RGBA color){
		font.drawStringWithShadow(text, x, y, color.argb());
	}
	...
	public String[] translate(String... strings){
		...
		for(int i = 0; i < strings.length; i++){
			trans[i] = StatCollector.translateToLocal(strings[i]);
		}
		return trans;
	}
